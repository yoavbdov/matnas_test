...
function exportCSV(
  students: Student[],
  allEnrollments: import("@/lib/types").Enrollment[],
  allTournaments: import("@/lib/types").Tournament[],
  allLeagueMembers: import("@/lib/types").LeagueGroupMember[],
) {
  const headers = [
    "שם פרטי",
    "שם משפחה",
...
export default function StudentsPage() {
  ...
  const displayedStudents = useMemo(() => {
    ...
    const filtered = students.filter((s) => {
      ...
      const computedStatus = computeStudentStatus(
        s.id,
        enrollments,
        tournaments,
        leagueGroupMembers,
      );
      if (statusFilter !== "הכל" && computedStatus !== statusFilter)
        return false;
      if (classFilter) {
        const enrolled = enrollments.some(
          (e) =>
            e.student_id === s.id &&
            e.class_id === classFilter &&
            e.status === "פעיל",
        );
    ...
    return [...filtered].sort((a, b) => {
      ...
      switch (sortCol) {
        ...
        case "age":
          ...
          break;
        case "grade": {
          const ga = effectiveGrade(
            a,
            settings.GRADE_FIRST_AGE,
            settings.GRADE_ADULT_AGE,
          );
          const gb = effectiveGrade(
            b,
            settings.GRADE_FIRST_AGE,
            settings.GRADE_ADULT_AGE,
          );
        ...
        case "rating":
          ...
          break;
        case "tournaments": {
          const ta = tournaments.filter((t) =>
            t.participant_ids.includes(a.id),
          ).length;
          const tb = tournaments.filter((t) =>
            t.participant_ids.includes(b.id),
          ).length;
          cmp = ta - tb;
          break;
        }
        case "league": {
          ...
          const getGroupName = (s: Student) => {
            ...
            if (!mem) return null;
            return (
              leagueGroups.find((g) => g.id === mem.group_id)?.name ?? null
            );
          };
        ...
        case "phone":
          ...
          break;
        case "classes": {
          const ca = enrollments.filter(
            (e) => e.student_id === a.id && e.status === "פעיל",
          ).length;
          const cb = enrollments.filter(
            (e) => e.student_id === b.id && e.status === "פעיל",
          ).length;
          cmp = ca - cb;
          break;
        }
        case "status": {
          const sa = computeStudentStatus(
            a.id,
            enrollments,
            tournaments,
            leagueGroupMembers,
          );
          const sb = computeStudentStatus(
            b.id,
            enrollments,
            tournaments,
            leagueGroupMembers,
  ...
  return (
    <PageShell title="שחקנים">
      ...
      <p className="text-xs text-gray-400 mb-3">
        {displayedStudents.length} שחקנים
      </p>
      ...
      {detailStudent && (
        <StudentDetailModal
          ...
          leagueGroups={leagueGroups}
          leagueGroupMembers={leagueGroupMembers}
          computedStatus={computeStudentStatus(
            detailStudent.id,
            enrollments,
            tournaments,
            leagueGroupMembers,
